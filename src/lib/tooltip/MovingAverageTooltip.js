	...
	_createClass(SingleMAToolTip, [{
		...
		value: function render() {
			var _props2 = this.props,
			    textFill = _props2.options.textFill,
		...
		value: function renderSVG(moreProps) {
			...
			return _react2.default.createElement(
				...
				options.map(function (each, idx) {
					...
					var yDisplayValue = yValue ? displayFormat(yValue) : displayFormat(each.recentValue) ;
					if(!viewHelper){
						yDisplayValue = displayFormat(each.recentValue);
					}
					return _react2.default.createElement(SingleMAToolTip, {
						key: idx,
						/*origin: [width * idx, 0],*/
						origin: [(each.defaultWidth), 0],
						color: each.stroke,
...
MovingAverageTooltip.propTypes = {
	...
	options: _propTypes2.default.arrayOf(_propTypes2.default.shape({
		...
		recentValue: _propTypes2.default.number.isRequired,
	}))
};
